    ...
    public:
        ...
        explicit VirtualStack(const size_t& p_stack_size, const size_t& p_safe_zone = 0)
          : stack_size(p_stack_size) {
            auto new_stack = std::make_unique<uint8_t[]>(stack_size + p_safe_zone);
            stack.swap(new_stack);
            stack_pointer = (StackPtr)((size_t)(stack.get()) + (stack_size + p_safe_zone));
        ...
        _NO_DISCARD_ _ALWAYS_INLINE_ size_t allocated() const { return (size_t)stack_pointer - (size_t)(stack.get()); }
